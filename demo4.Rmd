...
u_null <- function(x, y, n = 10) {
  ...
  sapply(1:n, FUN = function(i) {
    ...
    rand_x <- sample(xy, length(x), replace = FALSE)
    rand_y <- xy[!(xy %in% rand_x)]
...
* It is ok to compute two-tailed p = 2*one-tailed p, since we are interested in wether a treatment group grows better _or_ worse than control.
